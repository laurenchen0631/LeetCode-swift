...
func ManacherAlgorithm(s: String) -> String {
	...
	for i in 1 ..< n - 1 {
		...
		// T[left] == T[right]
		while T[T.index(index, offsetBy: 1 + P[i])] == T[T.index(index, offsetBy: -1 - P[i])] {
			P[i] += 1
		}
	...
	let left = s.index(s.startIndex, offsetBy: (center - 1 - P[center]) / 2)
    

	return s[left ..< s.index(left, offsetBy: maxLength)]
}
